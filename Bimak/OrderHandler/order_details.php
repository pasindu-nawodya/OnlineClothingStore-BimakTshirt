                            ...
                            <div class="col-md-12">
                                ...
                                <div class="table-responsive m-b-40">
                                    <table class="table table-borderless table-data3">
                                        ...
                                        <tbody>
                                            <?php
                                             $sql = "SELECT * FROM orders where oId not in(select oId from delivery_details)";
                                             $result = $con->query($sql); 
											if($result && $result -> num_rows> 0)
											{while($row = $result -> fetch_assoc()){ $_SESSION['duplicate'] = $row["oId"]; echo "<tr><td>".$row["oId"]."<td>". $row["cName"]."<td>".$row["No"]."<td>".$row["city"]."<td>".$row["province"]."</td></tr>"; }}else{ echo "no results";}  ?>
                                        </tbody>
                                    </table>
                                </div>
                                <!-- END DATA TABLE-->
									...
									<center>
									<form method = "get" action = "delivery_assign.php" target = "_blank">
								<input type = "submit" class="btn btn-outline-primary btn-lg btn-block" value = "Assign Deliveryman">
								</form> 
								<br>
								<form method = "get" action = "delivery_courier_assign.php" target = "_blank" >
								<input type = "submit" class="btn btn-outline-success btn-lg btn-block" value = "Assign Courier Service ">
								</form> 
                            </div>
